...
const ProjectCard = (props) => {
  const { title, description, link, img, languages, linkText } = props;
    ...
    <div className="group relative overflow-hidden border-2 border-black/50 dark:border-white/50 text-gray-700 bg-white m-2 rounded-xl w-60 h-60">
      <div className='group-hover bg-black/70 z-40 w-full h-full absolute transition-all duration-300'></div>
        ...
        <div className='flex items-center'>
          <Link to={link} target="_blank" class="inline-flex mr-4 items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            LIVE DEMO
            <svg class="rtl:rotate-180 w-3.5 h-3.5 ms-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
              <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9"/>
            </svg>
          </Link>
          <Link to={linkText} target="_blank">
